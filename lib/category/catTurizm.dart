...
class CatTurizm extends StatefulWidget {
  const CatTurizm({key}) : super(key: key);

  @override
  _CatTurizmState createState() => _CatTurizmState();
}

class _CatTurizmState extends State<CatTurizm> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text("Turizm",style: TextStyle(fontSize: 26.0,fontWeight: FontWeight.w600,color: Colors.white),),

      ),
      backgroundColor: Colors.white,
      body: StreamBuilder<QuerySnapshot>(
        stream: db.collection("newNews").where("kategori", isEqualTo: "Turizm").orderBy("id",descending: true).snapshots(),
        builder: (context, snapshot) {
          if (!snapshot.hasData) {
            return Center(child: CircularProgressIndicator());
          }
          List<DatabaseHelper> database = snapshot.data.documents
